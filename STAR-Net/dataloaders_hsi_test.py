...
class Dataset(Dataset):
    def __init__(self, root_dirs, transform=None, verbose=False):
        ...
        for cur_path in root_dirs:
            self.images_path += [path.join(cur_path, file) for file in listdir(cur_path) if
                                 file.endswith(('tif', 'png', 'jpg', 'jpeg', 'bmp', 'mat'))]
    ...
    def __getitem__(self, idx):
        ...
        #        image = hdf5storage.loadmat(img_name)['DataCube'].astype(np.float32)
        #image = hdf5storage.loadmat(img_name)['Noisy_Img'].astype(np.float32)
        image = hdf5storage.loadmat(img_name)['input'].astype(np.float32)
        if self.transform:
...
def get_gt(gt_path, img_name, verbose=False):
    ...
    #image = hdf5storage.loadmat(gt_path + img_name)['Img'].astype(np.float32)
    image = hdf5storage.loadmat(gt_path + img_name)['gt'].astype(np.float32)
